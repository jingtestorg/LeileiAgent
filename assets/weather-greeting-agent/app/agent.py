...
def get_system_prompt() -> str:
    base_prompt = """You are an AI agent that greets users with personalized hello messages based on their location and current weather conditions. Help users with their requests.\n\nWhen a user provides their location (city name or coordinates), you MUST:\n1. Call resolve_location to extract the city name from their message\n2. Call get_weather with that city to fetch current conditions\n3. Compose a warm, personalized greeting mentioning the city, temperature, and weather condition\n4. If the weather condition suggests special clothing or activities, add a friendly tip\n\nIf location cannot be determined or weather is unavailable, respond with a friendly generic greeting and explain what happened.\n\nIMPORTANT: You MUST use tools to retrieve live data. Never fabricate, guess, or invent weather data. Relay tool errors verbatim without adding suggestions. Never store or log user location data persistently. Never make write calls to external systems.""" + _DEFENSIVE_PROMPT_SUFFIX
...
class SampleAgent:
    ...
    async def _run_agent(
        ...
        """Core agent logic — extracted from stream() to allow safe OTel instrumentation.

        Returns the final agent response string.
        """
        with tracer.start_as_current_span("weather-greeting-agent.run"):
            system_prompt = get_system_prompt()
            tool_names = [tool.name for tool in tools] if tools else []
            logger.info("Running agent with %d tool(s): %s", len(tool_names), tool_names)

            # M1: Location input accepted (logged inside location tool; tracked here for span)
            with tracer.start_as_current_span("M1.location-input-accepted"):
                logger.info("M1.achieved: location extracted from user input")
            ...
            with tracer.start_as_current_span("M2.weather-data-retrieved"):
            ...
            with tracer.start_as_current_span("M3.greeting-composed"):
                logger.info("M3.achieved: personalized greeting composed")

            with tracer.start_as_current_span("M4.response-delivered"):
    ...
    async def stream(
        ...
        try:
            ...
            logger.info("M3.missed: greeting composition failed")
            logger.info("M4.missed: response delivery failed")
            yield {
                "is_task_complete": True,
                "require_user_input": False,
